...
export const ProductRow: React.FC<ProductRowProps> = ({ product, showAction, showDosage, onAddToCart, onClick, themeColor = 'green' }) => {
    ...
    return (
        <Card className={`flex items-center gap-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-800/50 ${borderColor} shadow-sm transition-all`} noPadding onClick={onClick}>
            <div className="p-3 pr-0"><img src={product.image_style} className="w-16 h-16 rounded-xl object-cover border dark:border-slate-700" alt={product.name} /></div>
            <div className="flex-1 py-3 pr-4 text-left">
                <div className="flex justify-between items-start">
                    <div>
                        <h4 className="font-bold text-[#111827] dark:text-slate-100 text-sm truncate">{product.name}</h4>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium truncate">{product.active_ingredient}</p>
                    </div>
                    <span className="font-bold text-sm shrink-0 ml-2" style={{ color: primaryColor }}>{product.price.toLocaleString()}đ</span>
                </div>
