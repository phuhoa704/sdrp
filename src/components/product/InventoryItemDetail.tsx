  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right duration-300">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-white max-w-md truncate">{item?.title || 'Loading...'}</h2>
          <p className="text-xs text-slate-500 font-bold">Chi tiết kho</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-8 text-sm pb-8">
          ...
          <Card className="bg-slate-900 border-slate-800 p-0">
            <div className="p-4 grid grid-cols-1 divide-y divide-slate-800">
              <div className="flex justify-between py-3 first:pt-0">
                <span className="font-bold text-slate-500">SKU</span>
                <span className="text-slate-300 font-medium">{item.sku || '-'}</span>
              </div>
              <div className="flex justify-between py-3">
                <span className="font-bold text-slate-500">Trong kho</span>
                <span className="text-slate-300 font-medium">{totalStocked} tại {locationCount} kho</span>
              </div>
              <div className="flex justify-between py-3">
                <span className="font-bold text-slate-500">Đặt trước</span>
                <span className="text-slate-300 font-medium">{totalReserved} tại {locationCount} kho</span>
              </div>
              <div className="flex justify-between py-3 last:pb-0">
                <span className="font-bold text-slate-500">Còn lại</span>
                <span className="text-slate-300 font-medium">{totalAvailable} tại {locationCount} kho</span>
              </div>
            </div>
          </Card>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-200">Kho</h3>
            ...
            <Card className="bg-slate-900 border-slate-800 p-0 overflow-hidden">
              <div className="p-2 flex justify-end gap-2 border-b border-slate-800">
                <button className="p-1.5 hover:bg-slate-800 rounded text-slate-500 transition-colors"><Filter size={14} /></button>
                <button className="p-1.5 hover:bg-slate-800 rounded text-slate-500 transition-colors"><SlidersHorizontal size={14} /></button>
              </div>

              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="text-[10px] uppercase text-slate-500 bg-slate-900/50 font-bold border-b border-slate-800">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-800 text-xs">
                    {levels.map(level => (
                      <tr key={level.id} className="group hover:bg-slate-800/30 transition-colors">
                        <td className="px-4 py-3 font-medium text-slate-300">
                          {level.stock_locations?.[0]?.name || 'Unknown Location'}
                        </td>
                        <td className="px-4 py-3 text-slate-400 font-medium">{level.reserved_quantity}</td>
                        <td className="px-4 py-3 text-slate-400 font-medium">{level.stocked_quantity}</td>
                        <td className="px-4 py-3 text-slate-400 font-medium">{level.available_quantity}</td>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-200">Reservations</h3>
              <Button variant="secondary" className="h-7 text-xs px-3">Thêm</Button>
            </div>
            <Card className="bg-slate-900 border-slate-800 min-h-[150px] flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="font-bold text-slate-300 mb-1 text-xs">No records</p>
