  ...
  return (
    ...
    >
      <div className="p-6 space-y-6 flex flex-col h-full overflow-hidden">
        {/* Header Table */}
        <div className="border border-slate-700 rounded-lg overflow-hidden shrink-0">
          <div className="grid grid-cols-2 divide-x divide-slate-700 bg-slate-900">
            <div className="p-3">
              ...
              <div className="text-sm text-slate-300 font-medium truncate">{inventoryItemTitle || '-'}</div>
            </div>
            <div className="p-3">
              <div className="text-[10px] font-bold text-slate-500 uppercase mb-1">SKU</div>
              <div className="text-sm text-slate-300 font-medium truncate">{inventoryItemSku || '-'}</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col min-h-0">
          <div className="flex justify-between items-center mb-2 shrink-0">
            <div>
              <h3 className="font-bold text-slate-200">Vị trí kho</h3>
          ...
          <div className="mb-4 relative shrink-0">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2 pl-9 pr-4 text-sm text-slate-300 focus:outline-none focus:border-indigo-500 transition-colors placeholder:text-slate-600"
              value={searchTerm}
              ...
              return (
                <div
                  ...
                  className={`
                                        ...
                                        ${isSelected ? 'bg-indigo-500/10 border-indigo-500/50' : 'bg-slate-900 border-slate-800 hover:border-slate-700'}
                                    `}
                >
                  <div className={`
                                        w-5 h-5 rounded border flex items-center justify-center mt-0.5 transition-colors
                                        ${isSelected ? 'bg-indigo-500 border-indigo-500 text-white' : 'bg-slate-800 border-slate-600'}
                                    `}>
                    {isSelected && <Check size={12} strokeWidth={3} />}
                  </div>
                  <div className="overflow-hidden">
                    <div className="text-sm font-bold text-slate-200 truncate">{loc.name}</div>
