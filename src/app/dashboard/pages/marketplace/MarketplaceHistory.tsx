...
import {
  ClipboardList, ArrowUpDown, Building2, X,
  MapPin, CreditCard, Printer,
  Download, Receipt, CheckCircle2, Box, ExternalLink
} from 'lucide-react';
import { Card } from '@/components/Card';
import { Button } from '@/components/Button';

export const MarketplaceHistory: React.FC = () => {
  ...
  const renderModal = () => {
    ...
    const modalContent = (
      <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 md:p-10 lg:p-16 animate-fade-in pointer-events-auto">
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl" onClick={() => setSelectedOrderDetails(null)} />

        <div className="relative w-full max-w-6xl h-[85vh] bg-white dark:bg-slate-900 rounded-[40px] overflow-hidden shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] animate-slide-up flex flex-col border border-white/10 dark:border-slate-800">
          ...
          <div className="p-10 md:p-12 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between shrink-0 gap-8 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
            ...
            <div className="flex gap-4 w-full md:w-auto">
              <Button variant="secondary" className="h-16 rounded-[20px] px-10 font-bold flex-1 md:flex-none dark:bg-slate-800 shadow-sm border-slate-200" onClick={() => alert("Đang tải đơn đặt hàng...")}>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between px-2">
        <h3 className="text-sm font-bold text-[#111827] flex items-center gap-2 dark:text-slate-100">
          <ClipboardList size={18} className="text-[#22C55E]" /> Lịch sử đơn sỉ
        </h3>
        <button className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl px-4 py-2.5 flex items-center gap-2 text-xs font-bold text-[#111827] dark:text-slate-100 shadow-sm">
          <ArrowUpDown size={14} className="text-[#22C55E]" /> Sắp xếp
        </button>
      </div>
